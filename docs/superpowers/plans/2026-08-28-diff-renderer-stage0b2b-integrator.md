...
- [ ] Wavefront and `PathTracer` agree within a **stated, derived** Monte Carlo bound on a probe-owned scene, and agreement improves with sample count.
  **NOT MET — recorded, not reinterpreted (2026-08-28).** `PathTracer` is a `VK_KHR_ray_tracing_pipeline` renderer; `GpuProbeContext` enables only `VK_KHR_ray_query` (`tests/diff/gpu_probe_context.cpp:202`), and this stage's Global Constraints forbid adding the extension. So `PathTracer` is unreachable from the probe and this criterion, as written, could not be satisfied by any work inside the stage's own constraints.
  What Task 6 delivered instead: an independent CPU path tracer written inside the probe from the definition of the estimator — Möller–Trumbore rather than a BVH, a different orthonormal basis, cosine-hemisphere sampling with no environment importance sampling, no CDF, no `pdfEnvMap` and no MIS, and its own RNG — sharing only the scene, environment and camera. The convergence half of the criterion IS met against that reference (rms ratios 0.5323 and 0.5153 against a predicted 0.5 on a 4× budget), and three perturbations were demonstrated to reject.
  **Corrected after review — do not misprice this deferral.** An earlier version of this note said cross-renderer parity was the stronger guarantee. It is not, on the axis that matters. `shaders/rt/pt_raygen.rgen` includes `rt/mis.glsl`, `rt/env_sampling.glsl`, `material/ggx_aniso.glsl` and `pbr_unpack.glsl` — **the same four headers the wavefront integrator builds on, by explicit design.** So a `PathTracer` ↔ wavefront comparison would have been blind to any bug in the balance heuristic, the environment CDF sampler and its pdf, the microfacet D/G/VNDF, or the roughness/metallic unpack. Those are exactly the components the CPU reference does *not* share. **On the estimator axis the delivered check is strictly stronger than the criterion it replaces.**
  What cross-renderer parity would still add is a *convention-and-orientation* gate: the CPU reference shares four host-side transcriptions with the GPU — the camera construction, the environment direction→texel inversion, the geometric-normal flip, and (via `oracleBsdfEval`) the BSDF itself. A second renderer would share none of them. That is a real and separate guarantee, and it is what the rescheduled work should be scoped to deliver — not "the real gate we skipped".
  Closing it needs the RT-pipeline extension in the probe plus a lighting-model reconciliation (`PathTracer::render` takes analytic light parameters the wavefront integrator has no concept of, which is the real blocker rather than the extension). Stage-sized.
