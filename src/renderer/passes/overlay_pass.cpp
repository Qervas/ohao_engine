...
bool OverlayPass::initialize(VkDevice device, VkPhysicalDevice physicalDevice) {
    m_device = device;
...
void OverlayPass::execute(VkCommandBuffer cmd, uint32_t /*frameIndex*/) {
    if (!m_gridEnabled) return;
    if (m_inputView == VK_NULL_HANDLE || m_depthView == VK_NULL_HANDLE) return;

    // Update descriptor set with current input/depth views
    std::array<VkDescriptorImageInfo, 2> imageInfos{};
    std::array<VkWriteDescriptorSet, 2> writes{};

    imageInfos[0].sampler = m_sampler;
    imageInfos[0].imageView = m_inputView;
    ...
    // Push constants
    GridPushConstants params{};
    params.invViewProj = m_invViewProj;
    params.cameraPos = glm::vec4(m_cameraPos, 0.0f);
    params.gridParams = glm::vec4(m_majorSpacing, m_minorSpacing, m_fadeDistance, 1.0f);

    vkCmdPushConstants(cmd, m_pipelineLayout,
                       VK_SHADER_STAGE_FRAGMENT_BIT,
                       0, sizeof(GridPushConstants), &params);
...
void OverlayPass::onResize(uint32_t width, uint32_t height) {
...
void OverlayPass::setCameraData(const glm::mat4& /*view*/, const glm::mat4& /*proj*/,
                                 const glm::mat4& invViewProj, const glm::vec3& cameraPos) {
    m_invViewProj = invViewProj;
    m_cameraPos = cameraPos;
}

bool OverlayPass::createOutputImage() {
    ...
    return vkCreateImageView(m_device, &viewInfo, nullptr, &m_outputView) == VK_SUCCESS;
}

void OverlayPass::destroyOutputImage() {
...
bool OverlayPass::createRenderPass() {
...
bool OverlayPass::createFramebuffer() {
    VkFramebufferCreateInfo framebufferInfo{};
    framebufferInfo.sType = VK_STRUCTURE_TYPE_FRAMEBUFFER_CREATE_INFO;
    framebufferInfo.renderPass = m_renderPass;
    framebufferInfo.attachmentCount = 1;
    framebufferInfo.pAttachments = &m_outputView;
...
bool OverlayPass::createDescriptors() {
    // Create sampler
    ...
    // Descriptor layout: input image + depth
...
bool OverlayPass::createPipeline() {
    VkShaderModule vertShader = loadShaderModule("overlay_grid.vert.spv");
    VkShaderModule fragShader = loadShaderModule("overlay_grid.frag.spv");
    ...
    pushConstant.size = sizeof(GridPushConstants);
