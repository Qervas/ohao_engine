...
bool AuroraPass::initialize(VkDevice device, VkPhysicalDevice physicalDevice) {
    m_device         = device;
    m_physicalDevice = physicalDevice;

    if (!createDescriptors()) {
        std::cerr << "AuroraPass: descriptor creation failed" << std::endl;
        return false;
    }
    if (!createPipelineResources()) {
        std::cerr << "AuroraPass: pipeline creation failed" << std::endl;
        return false;
    }
    std::cout << "AuroraPass: OK" << std::endl;
    return true;
}

void AuroraPass::cleanup() {
...
void AuroraPass::execute(VkCommandBuffer cmd, uint32_t /*frameIndex*/) {
    ...
    vkCmdPushConstants(cmd, m_pipelineLayout, VK_SHADER_STAGE_COMPUTE_BIT,
                       0, sizeof(AuroraParams), &params);

    uint32_t groupX = (m_width    + 7u) / 8u;
    uint32_t groupY = (m_height_px + 7u) / 8u;
...
void AuroraPass::onResize(uint32_t width, uint32_t height) {
    m_width     = width;
    m_height_px = height;
}

void AuroraPass::setHDROutput(VkImageView view, VkImage image) {
...
void AuroraPass::setDepthView(VkImageView view, VkSampler sampler) {
    if (view != m_depthView) {
        m_depthView = view;
...
bool AuroraPass::createDescriptors() {
...
bool AuroraPass::createPipelineResources() {
    return createComputePipeline(
        "compute_aurora.comp.spv",
        m_descriptorLayout,
        static_cast<uint32_t>(sizeof(AuroraParams)),
        m_pipeline, m_pipelineLayout);
}

void AuroraPass::updateDescriptors() {
