...
bool SnowPass::initialize(VkDevice device, VkPhysicalDevice physicalDevice) {
    m_device         = device;
    m_physicalDevice = physicalDevice;

    if (!createDescriptors()) {
        std::cerr << "SnowPass: descriptor creation failed" << std::endl;
        return false;
    }
    if (!createPipelineResources()) {
        std::cerr << "SnowPass: pipeline creation failed" << std::endl;
        return false;
    }
    std::cout << "SnowPass: OK" << std::endl;
    return true;
}

void SnowPass::cleanup() {
...
void SnowPass::execute(VkCommandBuffer cmd, uint32_t /*frameIndex*/) {
...
void SnowPass::onResize(uint32_t width, uint32_t height) {
    m_width  = width;
    m_height = height;
}

void SnowPass::setHDROutput(VkImageView view, VkImage image) {
...
bool SnowPass::createDescriptors() {
...
bool SnowPass::createPipelineResources() {
    return createComputePipeline(
        "compute_snow.comp.spv",
        m_descriptorLayout,
        static_cast<uint32_t>(sizeof(SnowParams)),
        m_pipeline, m_pipelineLayout);
}

void SnowPass::updateDescriptors() {
