...
bool GodRaysPass::initialize(VkDevice device, VkPhysicalDevice physicalDevice) {
    m_device         = device;
    m_physicalDevice = physicalDevice;

    if (!createDescriptors()) {
        std::cerr << "GodRaysPass: descriptor creation failed" << std::endl;
        return false;
    }
    if (!createPipelineResources()) {
        std::cerr << "GodRaysPass: pipeline creation failed" << std::endl;
        return false;
    }
    std::cout << "GodRaysPass: OK" << std::endl;
    return true;
}

void GodRaysPass::cleanup() {
...
void GodRaysPass::execute(VkCommandBuffer cmd, uint32_t /*frameIndex*/) {
    ...
    GodRayParams params{};
    params.sunScreenPos = m_sunScreenPos;
    params.intensity    = m_intensity;
    params.decay        = 0.97f;
    params.sunRadius    = 0.08f;
    params.exposure     = 1.0f;
    params.numSamples   = 32;
    params.padding      = 0.0f;

    vkCmdPushConstants(cmd, m_pipelineLayout, VK_SHADER_STAGE_COMPUTE_BIT,
                       0, sizeof(GodRayParams), &params);
...
void GodRaysPass::onResize(uint32_t width, uint32_t height) {
    m_width  = width;
    m_height = height;
}

void GodRaysPass::setHDROutput(VkImageView view, VkImage image) {
...
void GodRaysPass::setDepthView(VkImageView view, VkSampler sampler) {
    if (view != m_depthView) {
        m_depthView = view;
        // If caller provides null sampler, create our own
...
bool GodRaysPass::createDescriptors() {
...
bool GodRaysPass::createPipelineResources() {
    return createComputePipeline(
        "compute_godray.comp.spv",
        m_descriptorLayout,
        static_cast<uint32_t>(sizeof(GodRayParams)),
        m_pipeline, m_pipelineLayout);
}

void GodRaysPass::updateDescriptors() {
